...
def support(X, candidate):
    ...
    for i in range(X.shape[0]):
        if np.all(X[i, candidate[0]]) & np.all(X[i, candidate[1]]):
            tmp[i] = 1
    return np.mean(tmp)
...
def apriori(X, minimal_support):
    ...
    support_ = np.zeros(X.shape[1])   # поддержка каждого 1-элементного набора
    for i in range(X.shape[1]):
        support_[i] = np.sum(X[:, i]) / X.shape[0]
    ...
    for k in range(2, n+1):
        ...
        for candidate in multi_element_candidates:
            if support(X, candidate) > minimal_support:
                ...
                # accepted_candidates = np.append(accepted_candidates, candidate).reshape((accepted_candidates.shape[0], 1))
                accepted_candidates = np.append(accepted_candidates, candidate)
                accepted_candidates = accepted_candidates.reshape((accepted_candidates.shape[0], 1))
